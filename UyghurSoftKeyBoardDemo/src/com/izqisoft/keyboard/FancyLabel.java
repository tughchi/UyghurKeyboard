...
class fancyLabelDraw extends Drawable {
	...
	private void drawNumCenter(Canvas canvas) {
		...
		if (key.fancyLabel != null) {
			...
			//mPaint.setTypeface(Typeface.DEFAULT_BOLD);
			mPaint.setColor(Color.WHITE);
			canvas.drawText(letter, 0, 1, keyX - bounds.right - tW0 / 2 - 3,
					keyY - tMY, mPaint);

			letter[0] = getLetter(3);
			mPaint.getTextBounds(letter, 0, 1, bounds);
			canvas.drawText(letter, 0, 1, keyX - bounds.left + tW0 / 2 + 3,
					keyY - tMY, mPaint);
			...
			mPaint.getTextBounds(letter, 0, 1, bounds);
			tMX = (bounds.right + bounds.left) / (float) 2.0;
			canvas.drawText(letter, 0, 1, keyX - tMX, keyY - tMY + tH0 - 1,
					mPaint);
	...
	private void drawNumLeft(Canvas canvas) {
		...
		if (key.fancyLabel != null) {
			...
			//mPaint.setTypeface(Typeface.DEFAULT_BOLD);
			mPaint.setColor(Color.YELLOW);
			canvas.drawText(letter, 0, 1, key.width * horizontalPadding
					* (float) 0.2 - tMX, keyY - tMY, mPaint);
			...
			tMX = (bounds.right + bounds.left) / (float) 2.0;
			tW0 = (bounds.right - bounds.left);
			canvas.drawText(letter, 0, 1, keyX - tMX, keyY - tMY + tH0 / 2 - 1,
					mPaint);
			...
			mPaint.getTextBounds(letter, 0, 1, bounds);
			tMY = (pfm.ascent + pfm.descent) / (float) 2.0;
			canvas.drawText(letter, 0, 1, keyX - bounds.right - tW0 / 2 - 2,
					keyY - tMY, mPaint);

			letter[0] = getLetter(3);
			mPaint.getTextBounds(letter, 0, 1, bounds);
			canvas.drawText(letter, 0, 1, keyX - bounds.left + tW0 / 2 + 2,
					keyY - tMY, mPaint);
		}
	}
	...
	private void drawNumBehind(Canvas canvas) {
		...
		if (key.fancyLabel != null) {
			float keyX = key.width * horizontalPadding / 2;
			float keyY = key.height * verticalPadding / 2;
			mPaint.setColor(Color.rgb(233	, 233, 233));
			// Set size and correct it
			float textSize = key.height * verticalPadding / 2;
			mPaint.setTextSize(textSize);
			Paint.FontMetrics pfm = mPaint.getFontMetrics();
			...
			mPaint.getTextBounds(key.fancyLabel.toString(), 0, key.fancyLabel.toString().length(), bounds);
			tMX0 = (bounds.right + bounds.left) / (float) 2.0;
			tMY = (pfm.ascent + pfm.descent) / (float) 2.0;
			tMY0 = (bounds.top + bounds.bottom) / (float) 2.0;
			//mPaint.setTypeface(Typeface.DEFAULT_BOLD);
		
			canvas.drawText(key.fancyLabel.toString(), keyX - tMX0, keyY - tMY0, mPaint);
			...
			tMX = (bounds.right + bounds.left) / (float) 2.0;
			tW0 = (bounds.right - bounds.left);
			canvas.drawText(key.fancyLabel.toString(), 0, 1, keyX - tMX, keyY - tMY + tH0 / 2 - 1,
					mPaint);
			...
			mPaint.getTextBounds(letter, 0, 1, bounds);
			tMY = (pfm.ascent + pfm.descent) / (float) 2.0;
			canvas.drawText(key.fancyLabel.toString(), 0, 1, keyX - bounds.right - tW0 / 2 - 4,
					keyY - tMY, mPaint);

			letter[0] = getLetter(3);
			mPaint.getTextBounds(letter, 0, 1, bounds);
			canvas.drawText(key.fancyLabel.toString(), 0, 1, keyX - bounds.left + tW0 / 2 + 4,
					keyY - tMY, mPaint);
*/
			/*
			 * // Draw semi-transparent background again letter[0]=getLetter(0);
			 * mPaint.setTextSize( textSize * (float)2);
			 * //mPaint.setTypeface(Typeface.DEFAULT_BOLD);
			 * mPaint.setColor(0x20706040); canvas.drawText(letter, 0, 1,
			 * keyX-tMX0, keyY-tMY0, mPaint);
			 */
		}
	}
